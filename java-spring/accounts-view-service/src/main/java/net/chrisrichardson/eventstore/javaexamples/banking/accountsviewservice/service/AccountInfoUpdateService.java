...
import static net.chrisrichardson.eventstore.javaexamples.banking.accountsviewservice.service.MoneyUtil.toIntegerRepr;
...
public class AccountInfoUpdateService {
  ...
  public void create(String accountId, String customerId, String title, BigDecimal initialBalance, String description, Int128 version) {
    try {
      ...
      UpdateResult x = mongoTemplate.upsert(new Query(where("id").is(accountId).and("version").exists(false)),
              new Update()
  ...
  public void delete(String accountId) {
    accountInfoRepository.deleteById(accountId);
  }
  ...
  public void updateBalance(String accountId, String changeId, long balanceDelta, AccountChangeInfo ci) {
    UpdateResult x = mongoTemplate.updateMulti(new Query(where("id").is(accountId).and("version").lt(changeId)),
            new Update().
